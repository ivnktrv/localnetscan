...
=======
    ...
    #  Программа выводит таблицу с IP и MAC адресами отвеченных устройств. Также можно указать диапозон  #
    #  IP, тогда получится сканер локальной сети :). Обязательно вместе с .exe или .py файлом должен     #
    #  быть файл localnetscan-config.json, иначе ничего работать не будет. В файле конфигурации          #
    #  3 пункта:                                                                                         #
    #  {                                                                                                 #
...
>>>>>>> 45e11db0bd941bf66ba182acb784d450a54e0fdf
...
{                                                                                              
    "timeout": [время в секундах], <- отвечает за время ожидания ответа от устройства/устройств.  
    "writeDataInJson": true/false, <- отвечает за запись выходных данных в json файл.          
    "logo": true/false <- отвечает за вывод логотипа после запуска программы.                  
}             
